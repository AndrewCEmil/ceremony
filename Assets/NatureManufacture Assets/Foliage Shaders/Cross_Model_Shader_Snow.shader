Shader "NatureManufacture Shaders/Cross Model Shader Snow"
{
	Properties
	{
		...
		[NoScaleOffset]_SnowMaskR("Snow Mask (R)", 2D) = "black" {}
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult46 = lerp( tex2DNode3 , UnpackScaleNormal( tex2D( _SnowNormalRGB, uv_SnowAlbedoRGB ) ,_SnowNormalPower ) , saturate( ( ase_worldNormal * _Snow_Amount ) ).x);
			...
			float4 lerpResult56 = lerp( ( tex2DNode2 * _HealthyColor ) , ( tex2D( _SnowAlbedoRGB, uv_SnowAlbedoRGB ) - fixed4( appendResult76 , 0.0 ) ) , lerpResult150);
			o.Albedo = ( lerpResult56 * fixed4( _ColorAdjustment , 0.0 ) ).rgb;
			fixed3 temp_cast_4 = (0.0).xxx;
